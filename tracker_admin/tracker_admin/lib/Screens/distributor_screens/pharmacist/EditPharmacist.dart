...
class EditPharmacist extends StatefulWidget {
  final String name;
  final String email;
  ...
  @override
  _EditPharmacistState createState() => _EditPharmacistState();
}

class _EditPharmacistState extends State<EditPharmacist> {
  double width;
  double height;
  double safePadding;
    ...
    if (await Permission.storage.request().isGranted) {
      ...
      try {
        ...
        var snapshot = await _storage
            .ref('Pharmacist/${email.text}/index0.png')
            .putFile(image);
  ...
  editPharmacistInfo() async {
    ...
    try {
      ...
      firestore.collection("Pharmacist").doc(email.text).update({
        "name": name.text,
        "phoneNumber": phoneNumber.text,
        "image": uploadedFileURL,
        "EditedBy": {distributorEmail: Timestamp.now()},
      }).then((_) async {
        ...
        Fluttertoast.showToast(msg: 'Pharmacist updated Succesfully!');
        setState(() {
          _isLoading = false;
        });
  ...
  void initState() {
    ...
    currentPharmacistEmail = FirebaseAuth.instance.currentUser.email;
  }
